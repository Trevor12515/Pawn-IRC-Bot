...
#define IRC_USER 	"TrevorBot"
#define IRC_PWD  	""
#define IRC_RLN 	"Trevor12515"
#define IRC_MAIN	"#GT"
#define IRC_ADM     "#GT.admins"
#define IRC_JOIN 	"#GT"
...
IRCCMD:version(botid, channel[], user[], host[], params[]) {
	IRC_Say(bot, channel, "PAWN IRC Bot by Trevor12515 Version "#BOT_VERS);
...
{
    new string[128];
    format(string, sizeof(string), "The bot has been up for %d week(s), %d days, %d hours, %d minutes and %d seconds.", Weeks, Days, Hours, Minutes, Seconds);
    IRC_Say(bot, channel, string);
...
IRCCMD:cmds(botid, channel[], user[], host[], params[]) {
	IRC_Say(bot, channel, "Prefix: \""#IRC_PREFIX"\", Commands: restart, say, raw, uptime, version, cmds, commands, cmdhelp, commandhelp, pawn, wiki");
...
IRCCMD:cmdhelp(botid, channel[], user[], host[], params[]) {
	...
	else if (!strcmp(params, "uptime", true)) 		IRC_Say(bot, channel, "UPTIME - Uptime Info");
...
IRCCMD:wiki(botid, channel[], user[], host[], params[]) {
	...
	format(data, sizeof(data), "merc-gaming.com/wiki.php?lookup=%s&return=url", params);
...
IRCCMD:pawn(botid, channel[], user[], host[], params[]) {
	...
	format(data, sizeof(data), "merc-gaming.com/wiki.php?lookup=%s", params);
...
public OnGameModeExit()
{
	...
	IRC_Quit(bot, "Trevor12515's PAWN IRC Bot Exiting");
